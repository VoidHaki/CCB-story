...
import { runTransaction, SpinReward } from "@/lib/db";
...
export async function GET(req: Request) {
  try {
    ...
    const statusData = await runTransaction((db) => {
    ...
    return NextResponse.json({ error: "Unable to load spin status." }, { status: 500 });
  }
}
