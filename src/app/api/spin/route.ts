...
const SPIN_REWARDS = [
  ...
  { reward: "Lucky Bonus",  type: "bonus"    as const, value: 50,           icon: "✨", weight: 15 },
  { reward: "Better Luck!", type: "luck"     as const, value: "Better Luck",icon: "🍀", weight: 20 },
];
...
export async function POST(req: Request) {
  try {
    ...
    const result = await runTransaction((db) => {
      ...
      if (!table) {
        return { error: "Invalid table", status: 403 };
      }
    ...
    return NextResponse.json({ error: "Unable to complete spin. Please try again." }, { status: 500 });
  }
}
...
export async function GET(req: Request) {
  try {
    ...
    const spinRewards = await runTransaction((db) => {
      // If a specific tableId is requested, filter rewards for that table
      if (tableId) {
        return db.spinRewards.filter(r => r.tableId === String(tableId));
      }
      return db.spinRewards;
    }, true); // readonly

    return NextResponse.json(spinRewards);
  } catch (e) {
    console.error("Error in GET /api/spin:", e);
    return NextResponse.json({ error: "Unable to load spin history." }, { status: 500 });
  }
}
  ...
  try {
    ...
    const result = await runTransaction((db) => {
      ...
      if (reward.status !== "pending") {
        return { error: `Reward already ${reward.status}`, status: 409 };
      }

      // Check not expired
      if (new Date(reward.expiresAt).getTime() < Date.now()) {
        db.spinRewards[rewardIndex].status = "expired";
        return { error: "Voucher expired", status: 410 };
      }
      ...
      return { data: db.spinRewards[rewardIndex] };
    });
    ...
    return NextResponse.json({ error: "Unable to claim reward. Please try again." }, { status: 500 });
  }
}
